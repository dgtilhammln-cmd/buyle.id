...
Route::middleware(['auth'])->prefix('akun')->name('account.')->group(function () {
    ...
    Route::post('/pesanan/rating', [AccountController::class, 'submitRating'])->name('orders.rating');
    Route::get('/pesanan/{order}/unduh-tiket', [AccountController::class, 'downloadTicketReceipt'])->name('orders.ticket.download');
...
// Tracking endpoints — bio-click HARUS di atas wildcard {type}
...
Route::middleware(['auth', 'role:seller'])->prefix('creator')->name('creator.')->group(function () {
    ...
    Route::post('/products/scan-lynk', [\App\Http\Controllers\Creator\MenuScanController::class, 'scanLynk'])->name('products.scan-lynk');
...
// Products listing page: /{username}/produk
Route::get('/{username}/produk', [\App\Http\Controllers\BioProductsPageController::class, 'index'])
