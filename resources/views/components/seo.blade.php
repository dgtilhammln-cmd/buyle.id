...
@php
    ...
    // Auto-Fallback Logic with View section support
    $autoTitle = !empty($seoData['title'])
        ? $seoData['title']
        : (\View::hasSection('title')
            ? trim(\View::getSection('title'))
            : ($siteName . ' - ' . $tagline));

    $autoDesc = !empty($seoData['description'])
        ? $seoData['description']
        : (\View::hasSection('meta_description')
            ? trim(\View::getSection('meta_description'))
            : $shortDesc);

    $rawKeywords = !empty($seoData['keywords']) ? $seoData['keywords'] : (!empty($settings['meta_keywords_home']) ? $settings['meta_keywords_home'] : 'buyle.id, digital marketplace, produk digital, creator, jual beli digital');
    $competitorTerms = 'lynk.id, lynk id, agus leo halim, lynk.id indonesia, lynk';
    if (stripos($rawKeywords, 'lynk.id') === false) {
        $autoKeywords = $rawKeywords . ', ' . $competitorTerms;
    } else {
        $autoKeywords = $rawKeywords;
    }
...
<meta name="robots"
    content="{{ $seoData['robots'] ?? 'index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1' }}">
...
@if(!empty($seoData['hreflangs']))
    ...
    <link rel="alternate" hreflang="x-default"
        href="{{ $seoData['hreflangs']['en'] ?? ($seoData['hreflangs']['id'] ?? $canonical) }}">
@endif
