...
<head>
    ...
    <style>
        :root { --accent: #a1ff5a; --glass: rgba(255,255,255,0.06); --glass-border: rgba(255,255,255,0.1); --side: 24px; }
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        body { background: #000; color: #fff; font-family: 'Montserrat', sans-serif; min-height: 100vh; }
        .container { max-width: 500px; margin: 0 auto; padding-bottom: 60px; }

        /* Cover & Avatar */
        .cover-area { height: 160px; background: linear-gradient(135deg, #0b120c, #1a2e1c); background-size: cover; background-position: center; position: relative; }
        .profile-container { text-align: center; position: relative; margin-top: -50px; padding: 0 var(--side); }
        .avatar-wrap { width: 100px; height: 100px; border-radius: 50%; border: 4px solid #000; background: #111; overflow: hidden; display: inline-block; margin-bottom: 0.75rem; }
        .avatar-wrap img { width: 100%; height: 100%; object-fit: cover; }
        .profile-name { font-size: 1.35rem; font-weight: 900; color: #fff; letter-spacing: -0.02em; }
        .profile-bio { font-size: 0.82rem; color: rgba(255,255,255,0.6); margin: 0.4rem 0; line-height: 1.5; max-width: 340px; margin: 0.4rem auto; }
        ...
        .glass-btn .btn-title { font-size: 0.88rem; font-weight: 700; color: #fff; }
        ...
        <a href="{{ $block->url }}" target="_blank" class="glass-btn fade-up" style="animation-delay:{{ 0.35 + $i * 0.05 }}s">
            <div class="btn-icon">
                ...
                @elseif(!empty($block->data_json['image']))
                    <img src="{{ Str::startsWith($block->data_json['image'], 'http') ? $block->data_json['image'] : asset('storage/'.$block->data_json['image']) }}" alt="{{ $block->title }}" onerror="this.src='https://placehold.co/400x400/222/555?text=Link'">
