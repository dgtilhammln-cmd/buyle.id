...
<body>
    <div class="page-wrap">
        ...
        <div class="slider-container" id="sliderContainer">
            @if(count($images) > 0)
                @if(count($images) > 1)
                    <button class="slider-prev" onclick="slideMove(-1)"><i class="fas fa-chevron-left"
                            style="font-size:12px;"></i></button>
                    <button class="slider-next" onclick="slideMove(1)"><i class="fas fa-chevron-right"
                            style="font-size:12px;"></i></button>
                @endif
        ...
        <div class="seller-card">
            @if(!empty($config['avatar']))
                <img src="{{ asset('storage/' . $config['avatar']) }}" alt="{{ $config['name'] ?? $username }}"
                    class="seller-avatar">
            @else
            ...
            <div class="seller-info">
                ...
                <div class="seller-label">Creator buyle.id</div>
            </div>
            <a href="{{ route('bio.public', $username) }}"
                style="font-size:0.75rem; font-weight:700; color:{{ $accent }}; text-decoration:none;">Lihat Profil
                →</a>
        </div>
    </div>
