...
<body>
    ...
    <div class="container">
        ...
        <div class="cat-tabs-wrap fade-up" style="animation-delay:0.38s">
            <div class="cat-tabs" id="catTabs2">
                <div class="cat-tab active" data-tab="all" onclick="switchTab2('all',this)">Semua <span style="opacity:.6;font-size:.72rem;">{{ $totalAll }}</span></div>
                @if($hasAff)<div class="cat-tab" data-tab="rek" onclick="switchTab2('rek',this)">Rekomendasi <span style="opacity:.6;font-size:.72rem;">{{ $affBlocks->count() }}</span></div>@endif
                @if($hasCustom)<div class="cat-tab" data-tab="fis" onclick="switchTab2('fis',this)">Fisik <span style="opacity:.6;font-size:.72rem;">{{ $customProdBlocks->count() }}</span></div>@endif
                @if($hasBuyle)<div class="cat-tab" data-tab="dig" onclick="switchTab2('dig',this)">Digital <span style="opacity:.6;font-size:.72rem;">{{ $buyleBlocks->count() }}</span></div>@endif
            </div>
        </div>


        <div class="cat-panel active" id="tab2-all">
            <div class="product-grid">
                ...
                @foreach($customProdBlocks as $block)
                    ...
                    <a href="{{ $prodUrl }}" class="prod-card search-item bio-track-link" data-title="{{ $block->title }}" data-bio-block="{{ $block->id }}" data-bio-creator="{{ $profile->id }}">
                        <div class="prod-img-wrap"><div class="prod-number">{{ sprintf('%02d',$allNum) }}</div><img src="{{ $img }}" alt="{{ $block->title }}" onerror="this.src='https://placehold.co/400x400/fff/cbd5e1?text=Produk'"></div>
                ...
                @foreach($buyleBlocks as $block)
                    ...
                    @if($prod)
                    @php $pImg = !empty($prod->image) ? (Str::startsWith($prod->image, 'http') ? $prod->image : asset('storage/'.$prod->image)) : 'https://placehold.co/400x400/fff/cbd5e1?text=Produk'; @endphp
                    <a href="{{ $prodUrl }}" class="prod-card search-item bio-track-link" data-title="{{ $displayTitle }}" data-bio-block="{{ $block->id }}" data-bio-creator="{{ $profile->id }}">
                        <div class="prod-img-wrap"><div class="prod-number">{{ sprintf('%02d',$allNum) }}</div><img src="{{ $pImg }}" alt="{{ $prod->name }}" onerror="this.src='https://placehold.co/400x400/fff/cbd5e1?text=Produk'"></div>
        ...
        @if($hasAff)
        <div class="cat-panel" id="tab2-rek">
            <div class="product-grid">
        ...
        @if($hasCustom)
        <div class="cat-panel" id="tab2-fis">
            <div class="product-grid">
                @foreach($customProdBlocks as $i => $block)
                    ...
                    <a href="{{ $prodUrl }}" class="prod-card search-item bio-track-link" data-title="{{ $block->title }}" data-bio-block="{{ $block->id }}" data-bio-creator="{{ $profile->id }}">
                        <div class="prod-img-wrap"><div class="prod-number">{{ sprintf('%02d',$i+1) }}</div><img src="{{ $img }}" alt="{{ $block->title }}" onerror="this.src='https://placehold.co/400x400/fff/cbd5e1?text=Produk'"></div>
                        <div class="prod-info"><h3 class="prod-title">{{ $block->title }}</h3><div class="prod-price">@if(!empty($origPrice)&&$origPrice>$price)<span style="text-decoration:line-through;opacity:.5;font-size:.72rem;">Rp {{ number_format($origPrice,0,',','.') }}</span> @endif Rp {{ number_format($price,0,',','.') }}</div></div>
                    </a>
                @endforeach
            </div>
        ...
        @if($hasBuyle)
        <div class="cat-panel" id="tab2-dig">
            <div class="product-grid">
                @foreach($buyleBlocks as $i => $block)
                    ...
                    @if($prod)
                    <a href="{{ $prodUrl }}" class="prod-card search-item bio-track-link" data-title="{{ $displayTitle }}" data-bio-block="{{ $block->id }}" data-bio-creator="{{ $profile->id }}">
                        <div class="prod-img-wrap"><div class="prod-number">{{ sprintf('%02d',$i+1) }}</div>@if($prod->image)<img src="{{ asset('storage/'.$prod->image) }}" alt="{{ $prod->name }}">@else<img src="https://placehold.co/400x400/fff/cbd5e1?text=Produk" alt="No Image">@endif</div>
                        <div class="prod-info"><h3 class="prod-title">{{ $displayTitle }}</h3><div class="prod-price">@if(!empty($origPrice)&&$origPrice>$price)<span style="text-decoration:line-through;opacity:.55;font-size:.72rem;">Rp {{ number_format($origPrice,0,',','.') }}</span> @endif Rp {{ number_format($price,0,',','.') }}</div></div>
                    </a>
                    @endif
                @endforeach
        ...
        function switchTab2(tab, el) {
            document.querySelectorAll('#catTabs2 .cat-tab').forEach(t => t.classList.remove('active'));
            el.classList.add('active');
            document.querySelectorAll('[id^="tab2-"]').forEach(p => p.classList.remove('active'));
