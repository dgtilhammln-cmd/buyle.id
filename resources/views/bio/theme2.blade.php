        ...
        <div class="cat-panel active" id="tab2-all">
            <div class="product-grid">
                ...
                @foreach($customProdBlocks as $block)
                    @php $allNum++; $imgs=$block->data_json['images']??[]; $img=!empty($imgs[0])?asset('storage/'.$imgs[0]):(!empty($block->data_json['image'])?asset('storage/'.$block->data_json['image']):'https://placehold.co/400x400/fff/cbd5e1?text=Produk'); $prodUrl=route('bio.product.show',[$username,$block->data_json['slug']??$block->id]); $price=$block->data_json['price']??0; $origPrice=$block->data_json['original_price']??null; @endphp
                ...
                @foreach($buyleBlocks as $block)
                    ...
                    <a href="{{ $block->url }}" target="_blank" class="prod-card search-item bio-track-link" data-title="{{ $prod->name }}" data-bio-block="{{ $block->id }}" data-bio-creator="{{ $profile->id }}">
                        <div class="prod-img-wrap"><div class="prod-number">{{ sprintf('%02d',$allNum) }}</div>@if($prod->image)<img src="{{ asset('storage/'.$prod->image) }}" alt="{{ $prod->name }}">@else<img src="https://placehold.co/400x400/fff/cbd5e1?text=Produk" alt="No Image">@endif</div>
