...
<div style="max-width:640px;">
  ...
  <div style="background:#fff;border-radius:20px;box-shadow:0 2px 20px rgba(0,0,0,0.04);padding:2rem;">
    <form action="{{ $category ? route('admin.product-categories.update', $category) : route('admin.product-categories.store') }}" method="POST" enctype="multipart/form-data">
      ...
      <div style="margin-bottom:1.5rem;">
        ...
        <div style="display:flex;gap:1.5rem;align-items:center;">
          <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;">
            <input type="radio" name="icon_type" value="icon" {{ old('icon_type', $category?->icon_type ?? 'icon') === 'icon' ? 'checked' : '' }} onchange="toggleIconType()">
            <span style="font-size:.875rem;font-weight:600;color:#1E293B;">Pilih SVG Icon</span>
          </label>
          <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;">
            <input type="radio" name="icon_type" value="upload" {{ old('icon_type', $category?->icon_type ?? 'icon') === 'upload' ? 'checked' : '' }} onchange="toggleIconType()">
      ...
      {{-- SVG Icon Picker --}}
      <input type="hidden" name="icon_value" id="icon_value" value="{{ old('icon_value', $category?->icon_value ?? '') }}">
      <div id="icon_picker_section" style="margin-bottom:1.5rem; {{ old('icon_type', $category?->icon_type ?? 'icon') === 'icon' ? 'display:block;' : 'display:none;' }}">
      ...
      <div id="upload_picker_section" style="margin-bottom:1.5rem; {{ old('icon_type', $category?->icon_type ?? 'icon') === 'upload' ? 'display:block;' : 'display:none;' }}">
        <label style="font-size:.8rem;font-weight:700;color:#64748B;display:block;margin-bottom:.5rem;">UPLOAD GAMBAR ICON</label>
        @if(isset($category) && $category->icon_type === 'upload' && $category->icon_value)
            <div style="margin-bottom:1rem;">
                <img src="{{ asset('storage/' . $category->icon_value) }}" alt="Current Icon" style="width:64px;height:64px;object-fit:cover;border-radius:12px;border:1.5px solid #E2E8F0;">
            </div>
        @endif
        <input type="file" name="icon_upload" accept="image/png, image/jpeg, image/jpg, image/svg+xml, image/webp" style="width:100%;padding:.75rem 1rem;border:1.5px dashed #CBD5E1;border-radius:12px;font-size:.875rem;outline:none;background:#F8FAFC;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.25rem;">
        <div>
          ...
          <input type="text" name="badge" value="{{ old('badge', $category?->badge) }}" placeholder="Baru / Hot / Promo" style="width:100%;padding:.7rem 1rem;border:1.5px solid #E2E8F0;border-radius:12px;font-size:.9rem;outline:none;font-family:inherit;" onfocus="this.style.borderColor='#1eb349'" onblur="this.style.borderColor='#E2E8F0'">
        </div>
        <div>
          <label style="font-size:.85rem;font-weight:600;color:#374151;display:block;margin-bottom:.4rem;">Warna Badge</label>
          <input type="color" name="badge_color" value="{{ old('badge_color', $category?->badge_color ?? '#ef4444') }}" style="width:100%;height:44px;border:1.5px solid #E2E8F0;border-radius:12px;cursor:pointer;padding:.25rem;">
        </div>
      </div>
...
function toggleIconType() {
    ...
    if (type === 'icon') {
        document.getElementById('icon_picker_section').style.display = 'block';
        document.getElementById('upload_picker_section').style.display = 'none';
    } else {
        document.getElementById('icon_picker_section').style.display = 'none';
        document.getElementById('upload_picker_section').style.display = 'block';
    }
}

function selectIcon(key, element) {
