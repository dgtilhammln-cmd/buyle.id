...
@section('content')
    ...
    <div class="res-page">
        @if(session('success'))
            <div
                style="background:#F0FDF4;color:#15803D;padding:.875rem 1.25rem;border-radius:14px;margin-bottom:1.5rem;border:1px solid #BBF7D0;font-size:.825rem;font-weight:600;">
                ✓ {{ session('success') }}
            </div>
        @endif
        @if(session('error'))
            <div
                style="background:#FEF2F2;color:#991B1B;padding:.875rem 1.25rem;border-radius:14px;margin-bottom:1.5rem;border:1px solid #FECACA;font-size:.825rem;font-weight:600;">
                ✕ {{ session('error') }}
            </div>
        @endif
        ...
        <div class="res-stats-grid-10">
            ...
            <div class="res-stat-card">
                ...
                <div class="res-stat-val" style="color:#16A34A;">Rp {{ number_format($totalRevenueOverall, 0, ',', '.') }}
                </div>
            </div>
            ...
            <div class="res-stat-card">
                <div class="res-stat-top">
                    ...
                    <div class="res-stat-icon"
                        style="background:rgba(217, 119, 6, 0.08); color:#D97706; border-color:rgba(217, 119, 6, 0.15);">
                        <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="res-stat-val" style="color:#D97706;">Rp {{ number_format($totalAbandonedOverall, 0, ',', '.') }}
                </div>
            </div>
            ...
            <div class="res-stat-card" style="border-color:{{ $orphanData['count'] > 0 ? '#FECACA' : '#F1F5F9' }};">
                <div class="res-stat-top">
                    <div class="res-stat-lbl" style="color:{{ $orphanData['count'] > 0 ? '#DC2626' : '#64748B' }};">Ghost
                        Files</div>
                    <div class="res-stat-icon"
                        style="background:rgba(220, 38, 38, 0.08); color:#DC2626; border-color:rgba(220, 38, 38, 0.15);">
                        <svg width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="res-stat-val"
                    style="color:{{ $orphanData['count'] > 0 ? '#DC2626' : '#0F172A' }}; cursor:pointer;"
                    onclick="openGhostModal()">
                    {{ $orphanData['count'] }} <span
                        style="font-size:0.68rem; font-weight:600; color:#64748B;">({{ $orphanData['total_mb'] }} MB)</span>
                </div>
                @if($orphanData['count'] > 0)
                    <form action="{{ route('admin.creator-resources.clean-orphans') }}" method="POST" style="margin:0;">
                        @csrf
                        <button type="submit" class="btn-clean-orphan"
                            onclick="return confirm('Hapus {{ $orphanData['count'] }} berkas sampah terbuang ({{ $orphanData['total_mb'] }} MB) secara permanen dari server public storage?')">
                            Bersihkan Sampah
                        </button>
                    </form>
                @else
        ...
        <form method="GET" action="{{ route('admin.creator-resources.index') }}" class="res-filter-box">
            <div class="res-search-group">
                <input type="text" name="search" value="{{ $search }}"
                    placeholder="Cari nama, email, username, telepon, atau toko creator..." class="res-input">
            ...
            <div style="display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap;">
                ...
                <div style="display:flex; align-items:center; gap:0.3rem;">
                    ...
                    <select name="online_status" onchange="this.form.submit()" class="res-select">
                        ...
                        <option value="online_now" {{ $onlineFilter === 'online_now' ? 'selected' : '' }}>🟢 Online Sekarang
                        </option>
                        <option value="active_today" {{ $onlineFilter === 'active_today' ? 'selected' : '' }}>🟡 Aktif Hari
                            Ini</option>
                        <option value="active_week" {{ $onlineFilter === 'active_week' ? 'selected' : '' }}>🔵 Aktif Minggu
                            Ini</option>
                        <option value="inactive" {{ $onlineFilter === 'inactive' ? 'selected' : '' }}>⚪ Inaktif (> 7 Hari)
                        </option>
                    </select>
                </div>
                ...
                <div style="display:flex; align-items:center; gap:0.3rem;">
                    ...
                    <select name="sort_by" onchange="this.form.submit()" class="res-select">
                        <option value="storage_desc" {{ $sortBy === 'storage_desc' ? 'selected' : '' }}>Storage Terbesar
                            (Boros)</option>
                        <option value="storage_asc" {{ $sortBy === 'storage_asc' ? 'selected' : '' }}>Storage Terkecil
                        </option>
                        <option value="revenue_desc" {{ $sortBy === 'revenue_desc' ? 'selected' : '' }}>Omset Penjualan
                            Terbanyak</option>
                        <option value="abandoned_desc" {{ $sortBy === 'abandoned_desc' ? 'selected' : '' }}>Potensi Cart
                            Tertunda</option>
                        <option value="online_recent" {{ $sortBy === 'online_recent' ? 'selected' : '' }}>Aktivitas Online
                            Terbaru</option>
                        <option value="products_desc" {{ $sortBy === 'products_desc' ? 'selected' : '' }}>Jumlah Produk
                            Terbanyak</option>
                        <option value="blocks_desc" {{ $sortBy === 'blocks_desc' ? 'selected' : '' }}>Jumlah Bio Block
                            Terbanyak</option>
                    </select>
                </div>
            </div>
                        ...
                        <tr>
                                ...
                                <div class="res-user-info">
                                    <div class="res-avatar-wrap">
                                        @if($item['avatar_url'])
                                            ...
                                            <div class="res-avatar" style="display:none;">
                            ...
                            <td>
                                ...
                                <div style="font-size:0.72rem; color:#64748B; margin-top:0.1rem;">
                                    {{ number_format($item['asset_file_count']) }} berkas</div>
                            </td>
                            <td>
                            ...
                            <td>
                                @if($item['abandoned_cart_count'] > 0)
                                    <span style="font-size:0.825rem; font-weight:700; color:#D97706;"
                                        title="{{ $item['abandoned_cart_count'] }} barang tertunda di cart pembeli">
                                        Rp {{ number_format($abandoned, 0, ',', '.') }}
                                    </span>
                                    <div style="font-size:0.7rem; color:#D97706;">{{ $item['abandoned_cart_count'] }} item di cart
                                    </div>
                                @else
                            ...
                            <td>
                                <span style="font-size:0.78rem; font-weight:600; color:#1E293B;">
                                    {{ number_format($item['product_count']) }} produk &bull;
                                    {{ number_format($item['bio_blocks_count']) }} block
                                </span>
                            </td>
                            <td>
                                @if($rev > 0)
                            ...
                            <td style="text-align: right;">
                                <div style="display:inline-flex; gap:0.35rem; align-items:center; flex-wrap:nowrap;">
                                    ...
                                    @if($item['wa_link'])
                                        <a href="{{ $item['wa_link'] }}" target="_blank" class="btn-wa-nudge"
                                            title="Kirim Pesan WhatsApp Sapaan & Support Revenue ke Creator Ini">
                                            <svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24">
                                                <path
                                                    d="M12.031 0C5.385 0 0 5.385 0 12.031c0 2.124.553 4.197 1.604 6.014L0 24l6.138-1.611a11.97 11.97 0 005.893 1.541h.005c6.645 0 12.03-5.386 12.03-12.032C24.066 5.385 18.676 0 12.031 0zm0 22.033h-.004a9.98 9.98 0 01-5.09-1.396l-.365-.217-3.784.993 1.01-3.69-.238-.379a9.95 9.95 0 01-1.528-5.312c0-5.513 4.486-10 10-10 2.671 0 5.182 1.04 7.07 2.93 1.888 1.888 2.927 4.4 2.927 7.07 0 5.514-4.486 10-10 10z" />
                                            </svg>
                                            WA
                                        </a>
                                    @endif
                                    ...
                                    {{-- Compress Media --}}
                                    <form action="{{ route('admin.creator-resources.compress-all', $user->id) }}" method="POST"
                                        style="margin:0;">
                                        @csrf
                                        <button type="submit" class="btn-compress-sm"
                                            title="Kompresi Maksimal Berkas Media Creator Ini"
                                            onclick="return confirm('Kompresi semua gambar milik {{ addslashes($user->name) }}?')">
                                            <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5"
                                                viewBox="0 0 24 24">
                                                <path d="M4 14h6v6M20 10h-6V4M14 10l7-7M4 20l7-7" />
                                            </svg>
                                            Compress
                                        </button>
                                    </form>
    ...
    <div id="ghostModal" class="res-modal-overlay">
        <div class="res-modal-card">
            <div class="res-modal-header">
                <div>
                    <h3 style="font-size:1.05rem; font-weight:800; color:#0F172A; margin:0;">Pembersih Berkas Sampah
                        Terbuang (Ghost Files)</h3>
                    <p style="font-size:0.75rem; color:#64748B; margin:0.15rem 0 0 0;">Scan otomatis file fisik di public
                        storage yang tidak lagi direferensikan oleh database produk/bio/user.</p>
                </div>
                <button onclick="closeGhostModal()"
                    style="border:none; background:none; font-size:1.25rem; cursor:pointer; color:#64748B;">&times;</button>
            </div>
            <div class="res-modal-body">
                <div
                    style="background:#FEF2F2; border:1px solid #FEE2E2; padding:0.85rem 1rem; border-radius:12px; margin-bottom:1rem; display:flex; justify-content:space-between; align-items:center;">
                    <div>
                        <div style="font-size:0.85rem; font-weight:700; color:#991B1B;">Ditemukan {{ $orphanData['count'] }}
                            Ghost Files</div>
                        <div style="font-size:0.75rem; color:#7F1D1D;">Total ukuran berkas tak terpakai:
                            <strong>{{ $orphanData['total_mb'] }} MB</strong></div>
                    </div>
                    @if($orphanData['count'] > 0)
                        <form action="{{ route('admin.creator-resources.clean-orphans') }}" method="POST" style="margin:0;">
                            @csrf
                            <button type="submit" class="res-btn"
                                style="background:#DC2626; color:#ffffff; font-size:0.75rem; padding:0.5rem 1rem;"
                                onclick="return confirm('Hapus seluruh {{ $orphanData['count'] }} berkas sampah terbuang ({{ $orphanData['total_mb'] }} MB) secara permanen?')">
                                Hapus Semua Sampah
                            </button>
                        </form>
                    @endif
                            ...
                            <tbody>
                                @foreach(array_slice($orphanData['files'], 0, 50) as $f)
                                    <tr style="border-bottom:1px solid #F1F5F9;">
                                        <td
                                            style="padding:0.5rem 0.85rem; font-family:monospace; color:#334155; word-break:break-all;">
                                            {{ $f['path'] }}</td>
                                        <td style="padding:0.5rem 0.85rem; text-align:right; font-weight:700; color:#64748B;">
                                            {{ round($f['size'] / 1024, 1) }} KB</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
