...
<div style="background: #ffffff; border: 1px solid #E2E8F0; border-radius: 20px; padding: 1.5rem 2rem; margin-bottom: 1.75rem; display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 1.5rem; box-shadow: 0 4px 20px rgba(0,0,0,0.03);">
  <div>
    ...
    <span style="display: inline-flex; align-items: center; gap: 0.375rem; background: #f0fdf4; border: 1px solid #bbf7d0; color: #1eb349; font-size: 0.75rem; font-weight: 700; padding: 0.35rem 0.85rem; border-radius: 100px;">
  ...
  <div style="text-align: right; padding-left: 1.5rem; border-left: 1px dashed #E2E8F0;">
    ...
    <div id="realtime-time" style="font-family: 'Montserrat', monospace; font-size: 1.5rem; font-weight: 800; color: #1eb349; margin-top: 0.35rem; font-variant-numeric: tabular-nums;">00:00:00</div>
  </div>
</div>
...
<div style="margin-bottom:1.5rem; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
  <div>
    <h1 style="font-size:1.375rem;font-weight:800;color:#0F172A;margin:0 0 .25rem;letter-spacing:-.02em;">Overview Analytics</h1>
    <p style="font-size:.8125rem;color:#64748B;margin:0;">Metrik transaksi, visitor, creator, dan produk buyle.id</p>
  </div>

  {{-- FILTER PERIODE FORM --}}
  <form method="GET" action="{{ route('admin.dashboard') }}" id="filter-form" style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
    <input type="hidden" name="period" id="period-input" value="{{ $currentPeriod }}">

    {{-- Quick Filter Pills --}}
    <a href="{{ route('admin.dashboard', ['period' => '7d']) }}" class="filter-btn {{ $currentPeriod === '7d' ? 'active' : '' }}">7 Hari</a>
    <a href="{{ route('admin.dashboard', ['period' => '30d']) }}" class="filter-btn {{ $currentPeriod === '30d' ? 'active' : '' }}">30 Hari</a>
    <a href="{{ route('admin.dashboard', ['period' => '1y']) }}" class="filter-btn {{ $currentPeriod === '1y' ? 'active' : '' }}">1 Tahun</a>
    ...
    <button type="submit" class="filter-btn active" style="background: #1eb349; color: #fff; border-color: #1eb349;">
      <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M22 3H2l8 9.46V19l4 2v-8.54L22 3z"/></svg>
      Filter
    </button>
  </form>
</div>
...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1.25rem;margin-bottom:1.75rem;">
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;gap:1rem;">
      <div style="background:rgba(30, 179, 73, 0.1);border-radius:14px;width:52px;height:52px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
        <svg width="26" height="26" fill="none" stroke="#1eb349" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
      ...
      <div>
        ...
        <div style="font-size:1.75rem;font-weight:800;color:#0F172A;line-height:1.1;">{{ number_format($stats['visitor']) }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;gap:1rem;">
      ...
      <div>
        ...
        <div style="font-size:1.75rem;font-weight:800;color:#0F172A;line-height:1.1;">{{ number_format($stats['creators']) }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;gap:1rem;">
      ...
      <div>
        ...
        <div style="font-size:1.75rem;font-weight:800;color:#0F172A;line-height:1.1;">{{ number_format($stats['transactions']) }} <span style="font-size:0.75rem;font-weight:600;color:#94A3B8;">Order</span></div>
      </div>
    </div>
  </div>
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;gap:1rem;">
      ...
      <div>
        ...
        <div style="font-size:1.75rem;font-weight:800;color:#0F172A;line-height:1.1;">{{ number_format($stats['products']) }}</div>
      </div>
    </div>
  </div>
</div>
...
<div class="dash-card" style="margin-bottom: 1.75rem; padding: 1.75rem;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;flex-wrap:wrap;gap:1rem;">
    <div>
      <div style="font-size:.8rem;font-weight:700;color:#94A3B8;text-transform:uppercase;letter-spacing:.08em;margin-bottom:.25rem;">Grafik Performa Sistem</div>
      <div style="font-size:1.25rem;font-weight:800;color:#0F172A;">Trend Visitor, Creator, Transaksi &amp; Produk</div>
    </div>
...
<div style="display:grid;grid-template-columns:1.2fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;padding-bottom:0.75rem;border-bottom:1px solid #F1F5F9;">
      ...
      <span style="font-size:0.75rem;font-weight:700;background:#F1F5F9;color:#475569;padding:0.3rem 0.75rem;border-radius:50px;">Top 10</span>
    </div>
    ...
    <div class="order-scroll-box">
      @forelse($creatorsLeaderboard as $idx => $c)
        @php
          ...
          $rankText  = '#' . ($idx + 1);
        @endphp
        <div style="display:flex;align-items:center;justify-content:space-between;padding:0.75rem 0.5rem;border-bottom:1px solid #F8FAFC;gap:0.75rem;">
          <div style="display:flex;align-items:center;gap:0.75rem;overflow:hidden;">
            <div class="rank-badge {{ $rankClass }}">{{ $rankText }}</div>
            @if($c->avatar)
              <img src="{{ $c->avatar }}" style="width:38px;height:38px;border-radius:50%;object-fit:cover;border:1px solid #E2E8F0;flex-shrink:0;">
            @else
              <div style="width:38px;height:38px;border-radius:50%;background:#F1F5F9;color:#64748B;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:0.85rem;flex-shrink:0;border:1px solid #E2E8F0;">
                {{ strtoupper(substr($c->name, 0, 1)) }}
              </div>
            @endif
            <div style="overflow:hidden;">
              <a href="{{ route('store.show', ['slug' => $c->slug]) }}" target="_blank" style="font-size:0.875rem;font-weight:700;color:#0F172A;text-decoration:none;display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" onmouseover="this.style.color='#1eb349'" onmouseout="this.style.color='#0F172A'">
                {{ $c->name }}
              </a>
              <span style="font-size:0.75rem;color:#94A3B8;">/c/{{ $c->slug }} &bull; {{ $c->products_count }} Produk</span>
            </div>
          </div>

          <div style="display:flex;align-items:center;gap:0.5rem;flex-shrink:0;">
            <span style="font-size:0.75rem;font-weight:700;background:#F0FDF4;color:#166534;padding:0.3rem 0.6rem;border-radius:8px;border:1px solid #BBF7D0;display:inline-flex;align-items:center;gap:3px;">
  ...
  <div class="dash-card">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem;padding-bottom:0.75rem;border-bottom:1px solid #F1F5F9;">
      ...
      <a href="{{ route('admin.dashboard') }}" style="font-size:0.75rem;font-weight:700;color:#1eb349;text-decoration:none;">Refresh</a>
    </div>
    ...
    <div class="order-scroll-box">
      @forelse($recentOrders as $ord)
        ...
        @endphp
        <div style="padding:0.75rem;border:1px solid #F1F5F9;border-radius:12px;margin-bottom:0.65rem;background:#FAFAFA;transition:all 0.15s;" onmouseover="this.style.borderColor='#1eb349';this.style.background='#fff'" onmouseout="this.style.borderColor='#F1F5F9';this.style.background='#FAFAFA'">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.35rem;">
            <span style="font-size:0.8rem;font-weight:800;color:#0F172A;font-family:monospace;">#{{ $ord->order_number }}</span>
            <span style="font-size:0.7rem;font-weight:700;color:{{ $st[0] }};background:{{ $st[1] }};padding:0.2rem 0.5rem;border-radius:50px;">
              {{ $st[2] }}
            </span>
          </div>
          <div style="display:flex;align-items:center;justify-content:space-between;font-size:0.8rem;">
            <div style="color:#475569;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:180px;display:inline-flex;align-items:center;gap:3px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;margin-top:0.35rem;font-size:0.725rem;color:#94A3B8;">
