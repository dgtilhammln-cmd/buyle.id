...
@section('content')
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.75rem;">
    <div
      style="background:#fff;border-radius:16px;padding:1.25rem 1.5rem;box-shadow:0 2px 12px rgba(0,0,0,0.04);display:flex;align-items:center;gap:1rem;">
      <div
        style="width:44px;height:44px;background:rgba(59,130,246,0.1);border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
    ...
    <div
      style="background:#fff;border-radius:16px;padding:1.25rem 1.5rem;box-shadow:0 2px 12px rgba(0,0,0,0.04);display:flex;align-items:center;gap:1rem;">
      <div
        style="width:44px;height:44px;background:rgba(16,185,129,0.1);border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
      ...
      <div>
        <div style="font-size:1.5rem;font-weight:800;color:#1E293B;line-height:1;">
          {{ $faqs->where('is_published', true)->count() }}</div>
    ...
    <div
      style="background:#fff;border-radius:16px;padding:1.25rem 1.5rem;box-shadow:0 2px 12px rgba(0,0,0,0.04);display:flex;align-items:center;gap:1rem;">
      <div
        style="width:44px;height:44px;background:rgba(245,158,11,0.1);border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
      ...
      <div>
        <div style="font-size:1.5rem;font-weight:800;color:#1E293B;line-height:1;">
          {{ $faqs->where('is_published', false)->count() }}</div>
  ...
  <div id="view-list" class="view-container"
    style="background:#fff;border-radius:24px;box-shadow:0 2px 20px rgba(0,0,0,0.04);overflow:hidden;display:block;">
          ...
          <tr style="border-bottom:1px solid #F8FAFC;transition:background .15s;"
            ...
            <td style="padding:1.25rem 1.5rem;">
              <img src="{{ $a->image_url }}" alt="{{ $a->title }}"
                style="width:72px;height:50px;object-fit:cover;border-radius:10px;border:1px solid #E4E7F0;">
            </td>
            ...
            <td style="padding:1.25rem 1.5rem;max-width:300px;">
              <div
                ...
                {{ $a->title }}</div>
              <code
                style="font-size:.72rem;background:#F1F5F9;color:#3B82F6;padding:.2rem .5rem;border-radius:5px;font-family:'Courier New',monospace;margin-top:.25rem;display:inline-block;">/{{ $a->slug }}</code>
            </td>
            ...
            <td style="padding:1.25rem 1.5rem;">
              @if($a->category)
                <span
                  style="font-size:.75rem;font-weight:600;padding:.3rem .75rem;border-radius:8px;background:#F1F5F9;color:#475569;">{{ $a->category }}</span>
              @else
            ...
            <td style="padding:1.25rem 1.5rem;text-align:center;">
              ...
              @else
                <span
                  style="display:inline-flex;align-items:center;gap:.375rem;font-size:.75rem;font-weight:700;padding:.3rem .875rem;border-radius:100px;background:rgba(245,158,11,0.1);color:#F59E0B;">
            ...
            <td style="padding:1.25rem 1.5rem;text-align:center;">
              <div style="display:inline-flex;gap:.5rem;align-items:center;">
                {{-- Edit --}}
                <a href="{{ route('admin.faqs.edit', $a) }}" title="Edit"
        ...
        @empty
          <tr>
            <td colspan="7" style="padding:5rem;text-align:center;">
              <div
                style="width:64px;height:64px;background:#F1F5F9;border-radius:20px;display:flex;align-items:center;justify-content:center;margin:0 auto 1.25rem;">
              ...
              <a href="{{ route('admin.faqs.create') }}"
                style="display:inline-flex;align-items:center;gap:.5rem;margin-top:1.25rem;background:#3B82F6;color:#fff;font-size:.85rem;font-weight:700;padding:.625rem 1.25rem;border-radius:10px;text-decoration:none;">
  ...
  <div id="view-grid" class="view-container"
    ...
    @forelse($faqs as $a)
      <div
        ...
        <img src="{{ $a->image_url }}" alt="{{ $a->title }}"
          style="width:100%;height:160px;object-fit:cover;border-bottom:1px solid #F1F5F9;">
        <div style="padding:1.25rem;flex:1;display:flex;flex-direction:column;">
          ...
          <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:.75rem;">
            @if($a->category)
              <span
                style="font-size:.7rem;font-weight:600;padding:.2rem .5rem;border-radius:6px;background:#F1F5F9;color:#475569;">{{ $a->category }}</span>
            @endif
            <code
              style="font-size:.7rem;background:#F8FAFC;color:#3B82F6;padding:.2rem .5rem;border-radius:6px;border:1px solid #E2E8F0;">/{{ $a->slug }}</code>
          </div>

          <div
            style="display:flex;align-items:center;gap:.75rem;font-size:.75rem;color:#94A3B8;margin-bottom:1rem;font-weight:600;">
          ...
          <div
            ...
            <a href="{{ route('faqs.show', $a->slug) }}" target="_blank"
              style="font-size:.75rem;font-weight:700;color:#3B82F6;text-decoration:none;display:flex;align-items:center;gap:.25rem;">
              Lihat di website <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" />
                <polyline points="15 3 21 3 21 9" />
                <line x1="10" y1="14" x2="21" y2="3" />
            ...
            <div style="display:flex;gap:.375rem;">
              ...
              <form method="POST" action="{{ route('admin.faqs.destroy', $a) }}" onsubmit="return confirm('Hapus?')">
                @csrf @method('DELETE')
                <button type="submit" title="Hapus"
  ...
  <script>
    function switchView(type) {
      localStorage.setItem('admin.faqs_view', type);
    ...
    const savedView = localStorage.getItem('admin.faqs_view') || 'list';
