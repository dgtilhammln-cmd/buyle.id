...
@section('content')
    <div class="form-card">
        <form action="{{ route('creator.products.store') }}" method="POST" enctype="multipart/form-data" id="productForm">
            ...
            @if($errors->any())
                <div
                    style="background-color: #fee2e2; color: #b91c1c; padding: 1rem 1.5rem; border-bottom: 1px solid #fca5a5; font-size: 0.85rem; font-weight: 600;">
                ...
                <div class="form-grid">
                    <div class="form-group full">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="form-input"
                            placeholder="Contoh: Template Canva Premium Social Media Pack" required>
                        @error('name')<span class="form-error">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Kategori Platform</label>
                        <select name="product_category_id" id="catSelect" class="form-input"
                    ...
                    <div class="form-group">
                        <label class="form-label">Kelompok Produk (Creator)</label>
                        <select name="creator_group_id" class="form-input">
                            <option value="">— Tidak Masuk Kelompok —</option>
                            @foreach($groups as $grp)
                                <option value="{{ $grp->id }}" {{ old('creator_group_id') == $grp->id ? 'selected' : '' }}>
                                    {{ $grp->name }}</option>
                            @endforeach
                        </select>
                        <span class="form-hint"><a href="{{ route('creator.groups.index') }}"
                                style="color:#1eb349;text-decoration:none;">Kelola kelompok produk di sini</a></span>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="stock" value="{{ old('stock') }}" class="form-input"
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="short_desc" class="form-input" rows="2"
                            placeholder="Deskripsi singkat produk (maks 160 karakter)"
                            maxlength="160">{{ old('short_desc') }}</textarea>
            ...
            <div class="form-body">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="file_type" class="form-input" required>
                            ...
                            @foreach(['PDF', 'ZIP', 'RAR', 'Video (MP4)', 'Audio (MP3)', 'Ebook', 'Template (Canva/Notion)', 'Source Code', 'Spreadsheet (Excel/GSheets)', 'Document (Word)', 'Lainnya'] as $ft)
                                <option value="{{ $ft }}" {{ old('file_type') == $ft ? 'selected' : '' }}>{{ $ft }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group full">
                        ...
                        <input type="url" name="digital_resource" id="externalLink" value="{{ old('digital_resource') }}"
                        ...
                        <span class="form-hint">
                            ...
                            {{ implode(', ', array_slice($allowedDomains, 0, 6)) }}{{ count($allowedDomains) > 6 ? ', dan lainnya' : '' }}
                        </span>
                    ...
                    <div class="form-group full">
                        ...
                        <input type="url" name="tiktok_video_url" value="{{ old('tiktok_video_url') }}"
                            class="form-input" placeholder="https://www.tiktok.com/@username/video/123456789"
                            autocomplete="off">
            ...
            {{-- 1. Thumbnail Utama Produk (Wajib) --}}
            <div class="form-section-title">
                <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                Thumbnail Utama Produk (Wajib 1 Foto)
            </div>
            <div class="form-body">
                <div class="form-group">
                    ...
                    <div id="thumbPreviewWrap" style="display:none; margin-bottom:1rem;">
                        <div style="position:relative; display:inline-block; border-radius:12px; overflow:hidden; border:2px solid #1eb349; box-shadow:0 4px 12px rgba(30,179,73,0.15);">
                            <img id="thumbPreviewImg" src="" style="width:110px; height:110px; object-fit:cover; display:block;">
                            ...
                            <button type="button" onclick="removeThumbnail()" style="position:absolute; top:4px; right:4px; background:#ef4444; color:#fff; border:none; border-radius:50%; width:24px; height:24px; cursor:pointer; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 6px rgba(0,0,0,0.2);" title="Hapus / Ganti Thumbnail">
            ...
            <div class="form-body">
                <div class="form-group">
                    <label class="form-label">Upload Foto Galeri / Screenshot Detail</label>
                    ...
                    <div class="img-upload-area" id="galleryDropzone" onclick="document.getElementById('gallery-input').click()" style="margin-bottom:0.75rem;">
                        <div id="galleryPlaceholder">
                            ...
                            <p style="font-size:0.8rem; color:#64748B; margin-top:0.4rem; font-weight:600;">Klik untuk pilih hingga 6 foto galeri pendukung</p>
                ...
                <div class="form-grid">
                    <div class="form-group full">
                        ...
                        <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="form-input"
                            placeholder="Otomatis menggunakan nama produk jika kosong">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="meta_desc" class="form-input" rows="2"
                            placeholder="Otomatis menggunakan deskripsi singkat jika kosong">{{ old('meta_desc') }}</textarea>
                    ...
                    <div class="form-group full">
                        ...
                        <input type="text" name="meta_keywords" value="{{ old('meta_keywords') }}" class="form-input"
                            placeholder="e.g. preset lightroom, ebook bisnis">
    ...
    <style>
        ...
        .toggle-switch input:checked+.toggle-slider-s {
            background: #1eb349;
        }

        .toggle-switch input:checked+.toggle-slider-s::before {
...
@section('scripts')
    <script>
        ...
        function renderGalleryList() {
            ...
            if (galleryFilesDT.files.length > 0) {
                ...
                Array.from(galleryFilesDT.files).forEach((file, index) => {
                    ...
                    badge.style.cssText = 'position:absolute; bottom:0; left:0; right:0; background:rgba(0,0,0,0.65); color:#fff; font-size:9px; font-weight:600; text-align:center; padding:2px 0;';
        ...
        // ── 4. Real-time Digital Link Validator ──
        let linkTimer;
